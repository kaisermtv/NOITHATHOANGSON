...
public class DataMenu : DataClass
{
    ...
    public DataTable getList(int id = 0, int type = 1, bool countchild = false)
    {
        ...
        {
            SqlCommand Cmd = this.getSQLConnect();
            Cmd.CommandText = "SELECT P.ID,P.MenuID,P.NAME,P.LINK,P.IORDER" + (countchild ? ",(SELECT COUNT(*) FROM tblMenu WHERE PID = P.ID) AS COUNTCHILD" : "") + " FROM tblMenu AS P WHERE 1=1 ";
